...
namespace LinHong.Lib.Trigger.Requisitions
{
    public class EndFormTrigger : ICallbackTriggerPlugin
    {
        public string GetFormResult(ApplyTask applyTask)
        {
            ...
            //service.UpdateRequisitionsBuy_Unit(applyTask.FormNumber, buy_unit);
            if (applyTask.FormResult == Ede.Uof.WKF.Engine.ApplyResult.Adopt)
                service.UpdateRequisitions(applyTask.FormNumber, "1");
            else if (applyTask.FormResult == Ede.Uof.WKF.Engine.ApplyResult.Reject)
                service.UpdateRequisitions(applyTask.FormNumber, "4");

            string updateTime = DateTime.Now.ToString("yyyyMMddHHmmss");
            service.UpdateRequisitionsTime(applyTask.FormNumber, updateTime);

            Logger.Write("SW_InfoLog", string.Format("{0} 申請結案: {1}", applyTask.Task.FormName, applyTask.FormNumber));

            //表單標準明細代碼
            string stdFieldId = "std_purchaseDetail";
            //表單客製明細代碼
            string custFieldId = "purchaseDetail";
            //客製明細XML Cell Element
            string cellElement = "PRODUCT_GRIDITEM";
            //客製細XML Cell Attribute
            List<string> lstCellAttr = new List<string>() { "Part_Seq", "Part_No", "part_name", "part_name", "part_type", "price", "unit", "purchased_qty2", "remark", "require_qty" };
