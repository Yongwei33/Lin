    ...
    public class OnFlowTrigger : ICallbackTriggerPlugin
    {
        public string GetFormResult(ApplyTask applyTask)
        {
            if (applyTask.SignResult == SignResult.ReturnToAppliant)
            {
                ...
                service.DeletePR(applyTask.FormNumber);
                Logger.Write("SW_InfoLog", string.Format("{0}退簽:{1}", applyTask.Task.FormName, applyTask.FormNumber));
            }

            if (applyTask.SignResult == SignResult.Approve &&
                applyTask.SiteCode == "" &&
                applyTask.Task.CurrentSite.CurrentNode.OriginalSignerId == applyTask.Task.Applicant.UserGUID)
            {
                RequisitionsService service = new RequisitionsService();
            ...
            /*if (applyTask.SignResult == SignResult.Approve && applyTask.SiteCode == "qwe")
            {
                Logger.Write("SW_InfoLog", string.Format("qwe123"));
            }*/
                return "";
        }
